...
export class RolesComponent implements OnInit {
  ...
  filtrarRoles(event: { type: string; term: string }): void {
    ...
    this.roles.filterPredicate = (data: Rol, filter: string) => {
      if (type === 'id') {
        return data.IdRol.toString().includes(filter);  // Si busca por ID
      } else if (type === 'nombreRol') {
        return data.NombreRol.toLowerCase().includes(filter);  // Si busca por nombre
      }
      return false;  // Si no coincide ningún tipo
    };

    this.roles.filter = searchTerm;
